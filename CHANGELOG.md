...
- `comm__send` takes an optional `attachments` array of local file paths. A comm opts in with `:send-attachments? true` on its `:isaac.agent/comm` manifest entry; the tool refuses attachments for any other comm, and refuses a path that is missing, not a regular file, or outside the turn's allowed directories (the fs-tools check), queueing nothing. Relative paths resolve against the session cwd; the queued delivery record carries `:attachments [<path> …]` and the worker hands it to the comm's `send!` unchanged (isaac-o9h4).
