Feature: Compaction request shaping — low effort, sized to the window, and one retry at half size
  ...
  layer), never larger than the model's context window, and when a summary
  request is dropped by the provider it retries once at half size before
  counting a failure.
  ...
  Scenario: a history that fits the window is summarized in a single request
    Given the isaac EDN file "config/models/local.edn" exists with:
      | path           | value      |
      | model          | test-model |
      | provider       | grover     |
  ...
  Scenario: a summary prompt larger than the window is compacted in chunks that fit the window
    Given the isaac EDN file "config/models/local.edn" exists with:
      | path           | value      |
      | model          | test-model |
      | provider       | grover     |
