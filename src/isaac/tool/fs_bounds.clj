...
(defn- config-path? [root file-path]
  (boolean
    (and root file-path
         (some #(path-inside? % file-path) (config-directories root)))))
    ...
    (let [args        (string-key-map args)
      ...
      (when session
        (let [cfg         (loader/snapshot "tool fs-bounds: directory policy")
              crew-id     (or (get args "caller_crew") (:crew session) (defaults/crew-id cfg))
              ...
              root        (root args)]
          (when (or (config-path? root resolved)
                    (not (names/path-allowed? global-dirs crew-dirs resolved ctx)))
