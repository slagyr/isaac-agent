...
(describe "tool fs bounds"
  ...
  (it "rejects path traversal that escapes an allow prefix"
    (let [mem           (fs/mem-fs)
          session-store (store/create nil :memory)]
      (nexus/-with-nexus {:root "/isaac-state" :sessions {:store session-store} :fs mem}
        (store/open-session! session-store "fence-test" {:crew "main" :cwd "/work/project"})
        (config/dangerously-install-config!
          ...
          "spec")
        (let [denied (sut/ensure-path-allowed {"session_key" "fence-test"}
                                              "/isaac-state/crew/main/../../etc/passwd")]
          (should-not (nil? denied))
          (should (:isError denied))))))

  (it "denies the config directory even when quarters is granted"
    (let [mem           (fs/mem-fs)
          session-store (store/create nil :memory)]
      (nexus/-with-nexus {:root "/isaac-state" :sessions {:store session-store} :fs mem}
        ...
        (config/dangerously-install-config!
          ...
          "spec")
        (let [denied (sut/ensure-path-allowed {"session_key" "fence-test"}
                                              "/isaac-state/config/crew/main.edn")]
          (should-not (nil? denied))
          (should (:isError denied))))))

  (it "evaluates a symlink against its resolved path, not the link location"
    (let [tmp           (str (bfs/create-temp-dir "isaac-ukg4-"))
          cwd           (str tmp "/project")
          outside       (str tmp "/outside/secret.txt")
          link          (str cwd "/link.txt")
          session-store (store/create nil :memory)]
      (try
        (bfs/create-dirs cwd)
        (bfs/create-dirs (bfs/parent outside))
        (spit outside "nope")
        (bfs/create-sym-link link outside)
